...
def update_order(order_id):
    ...
    total_cost = updated_data('total_cost')
    cost_paid = updated_data('cost_paid')
    client_id = updated_data.get('client_id')
    appt_id = updated_data.get('appt_id')
    order = Order.update_order(address, num_of_windows, num_of_doors, notes, total_cost, cost_paid, appt_id, client_id)
    return jsonify(order.to_dict())